-- Videography Database Setup for TempahanPhotoStudio
-- Run this script in your SQL Server Management Studio

USE TempahanPhotoStudio;
GO

-- Check if packages table exists, if not create it
...
-- Check if sub_packages table exists, if not create it
...
-- Clear existing videography data (optional - remove if you want to keep existing data)
DELETE FROM sub_packages WHERE package_id IN (SELECT id FROM packages WHERE category = 'Videography');
DELETE FROM packages WHERE category = 'Videography';
GO
...
-- Commercial Videography Packages
...
-- Specialized Videography Packages
...
GROUP BY p.id, p.package_name, p.event, p.duration, p.category
ORDER BY p.package_name;
GO

-- Show total counts
SELECT 
    'Videography Packages' as TableName, 
    COUNT(*) as RecordCount 
FROM packages 
WHERE category = 'Videography'
UNION ALL
SELECT 
    'Videography Sub-Packages', 
    COUNT(*) 
...
PRINT 'Videography database setup completed successfully!';
PRINT 'Total videography packages added: ' + CAST((SELECT COUNT(*) FROM packages WHERE category = 'Videography') AS NVARCHAR(10));
PRINT 'Total videography sub-packages added: ' + CAST((SELECT COUNT(*) FROM sub_packages sp JOIN packages p ON sp.package_id = p.id WHERE p.category = 'Videography') AS NVARCHAR(10));
GO
